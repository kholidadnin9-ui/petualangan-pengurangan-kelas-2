...
import heroImg from "../assets/hero-characters.png";
import monkeyImg from "../assets/monkey-happy.png";
...
export default function HomeScreen({ onStart, onPickLevel, bestScores }: HomeScreenProps) {
    ...
    <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-2xl flex-col items-center px-4 pb-8 pt-5">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            <div className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-full border-4 border-amber-700 bg-white/95 shadow-lg">
              <img src={heroImg} alt="" className="h-20 w-20 -mt-3 object-contain" />
            </div>
          </div>
          <div className="wood-plank px-4 py-2">
            <div className="text-xs leading-none opacity-90">Halo, Pejuang Hebat!</div>
            <div className="text-sm font-black leading-tight">Siap berpetualang?</div>
          </div>
        </div>
      ...
      <div className="relative mt-2 w-full max-w-xl px-4 py-2 fade-up">
        ...
        <div
          ...
          style={{
            background: "linear-gradient(180deg, #d4934f 0%, #a66328 45%, #6b3615 100%)",
            boxShadow:
        ...
        >
          ...
          <div className="my-1.5 inline-flex items-center gap-1.5 rounded-full bg-amber-950/50 px-3.5 py-0.5 shadow-inner">
            <span className="text-[11px] font-extrabold tracking-wide text-amber-200">
              Created by: <span className="text-white underline decoration-amber-300">Widodo guru sd</span>
            </span>
          </div>
      ...
      <div className="relative mt-3 flex w-full items-end justify-center">
        ...
        <img
          ...
          style={{ filter: "drop-shadow(0 10px 14px rgba(0,0,0,0.2))" }}
        />
      </div>

      {/* Info planks */}
      <div className="mt-3 flex w-full max-w-md flex-col gap-2">
        <InfoPlank icon="⭐" text={<>5 Level Seru · {totalStars} Bintang</>} />
        <InfoPlank icon="🏆" text={<>Belajar Sambil Bermain</>} />
        <InfoPlank icon="❤️" text={<>Jadi Lebih Hebat!</>} />
      </div>
...
function InfoPlank({ icon, text }: { icon: string; text: React.ReactNode }) {
  return (
    <div
      className="relative mx-auto w-full max-w-xs rounded-2xl py-2 pl-12 pr-4 text-center text-white shadow-lg"
      style={{
        background: "linear-gradient(180deg, #c68642 0%, #9a5a25 50%, #6b3615 100%)",
        boxShadow:
          "inset 0 2px 0 rgba(255,255,255,0.3), inset 0 -4px 0 rgba(0,0,0,0.25), 0 5px 0 rgba(0,0,0,0.2), 0 10px 18px rgba(0,0,0,0.2)",
      }}
    >
      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-2xl drop-shadow">{icon}</span>
      <span className="font-extrabold text-sm sm:text-base">{text}</span>
    </div>
  );
}
