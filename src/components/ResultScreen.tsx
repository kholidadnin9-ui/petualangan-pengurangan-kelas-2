...
import castleTrophyImg from "../assets/castle-trophy.png";
...
export default function ResultScreen({
  ...
  if (allLevelsComplete) {
    return <FinalWinScreen onRetry={handleRetry} onHome={handleHome} score={score} />;
  }
  ...
  return (
    <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-xl flex-col items-center justify-center px-4 py-8">
      ...
      <div className="fade-up relative w-full">
        <Leaf className="absolute -left-6 -top-6 w-20" rotate={-15} />
        <Leaf className="absolute -right-6 -top-6 w-20" rotate={15} flip />
        <div
          className="mx-auto rounded-3xl p-6 text-center text-white sm:p-8"
          style={{
            background: "linear-gradient(180deg, #d4934f 0%, #a66328 50%, #6b3615 100%)",
            boxShadow:
        ...
        >
          ...
          <h2 className="mt-1 text-3xl font-black sm:text-4xl" style={{ textShadow: "0 3px 0 rgba(0,0,0,0.35)" }}>
            Selesai! 🎉
          </h2>

          {/* Parchment score card inside */}
          <div className="parchment mt-5 p-5">
            <div className="flex items-center justify-center gap-2">
              {[1, 2, 3].map((n) => (
                <span key={n} className={`big-star ${n <= stars ? "" : "dim"}`}>
                  ⭐
                </span>
              ))}
            </div>
            ...
            <p className="mt-1 text-sm font-bold text-amber-800">jawaban benar</p>
            <p className="mt-3 text-base font-extrabold text-amber-900">{message}</p>
          </div>
          ...
          <div className="mt-5 flex flex-col gap-3">
            {hasNext && score >= 6 && (
              <button className="game-btn btn-yellow w-full px-6 py-3 text-xl" onClick={handleNext}>
                Level Berikutnya →
              </button>
            )}
            <button className="game-btn btn-green w-full px-6 py-3 text-lg" onClick={handleRetry}>
              🔄 Main Lagi
            </button>
            <button className="game-btn btn-blue w-full px-6 py-3 text-lg" onClick={handleHome}>
              🏠 Menu
            </button>
          </div>
        </div>
  ...
  return (
    <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-xl flex-col items-center justify-center px-4 py-8">
      ...
      <div className="fade-up relative w-full text-center">
        <img src={castleTrophyImg} alt="" className="mx-auto w-56 object-contain drop-shadow-2xl sm:w-72" />
        <h2
        ...
        <p className="mt-2 text-lg font-extrabold text-white drop-shadow" style={{ textShadow: "0 2px 0 rgba(0,0,0,0.4)" }}>
        ...
        >
          ...
          <span className="wood-nail bl" />
          <span className="wood-nail br" />
          <p className="text-lg font-black">🏆 Kamu adalah</p>
          <p className="text-2xl font-black" style={{ textShadow: "0 2px 0 rgba(0,0,0,0.35)" }}>
            Pahlawan Pengurangan!
          </p>
          <p className="mt-2 text-sm font-bold opacity-90">Skor terakhir: {score}/10</p>
        </div>

        <div className="mt-5 flex flex-col gap-3">
          <button className="game-btn btn-yellow w-full px-6 py-3 text-xl" onClick={onRetry}>
            🔁 Main Lagi
          </button>
          <button className="game-btn btn-blue w-full px-6 py-3 text-lg" onClick={onHome}>
            🏠 Menu
          </button>
        </div>
      </div>
